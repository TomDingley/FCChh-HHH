...
For leptonic decays, an additional unknown of $m_{\text{miss}}$ is included --- amounting to 7 unknowns.
...
By scanning over missing-momentum vector azimuthal angles, $(\phi_1, \phi_2)$, the system is fully specified. 
...
Not all real solutions of the $p_z$ quadratic are equally likely, and knowledge of the expected $\tau$-decay kinematics can be used to guide the algorithm towards the most probable visible and missing momentum configuration.

Before introducing this reweighting, the baseline algorithmic performance can be quantified using the unweighted distributions. Likewise, for later optimisation studies, the performance is illustrated using the signal process together with the dominant $t\bar{t}b\bar{b}$ background.