...
- Prints best-fit xhat and [left,right] crossings for dNLL = 0.5 (68% CL) and 1.92 (95% CL).
...
Usage examples:
  ...
  python compare_k4_scans.py \
    ...
    --out k4_compare.pdf \
    /path/to/NLLscan_k4_curve.root \
    /path/to/NLLscan_k4_curve_scenI.root \
    /path/to/NLLscan_k4_curve_scenII.root
"""
...
# ---------------------------
def plot_compare_k4(files: List[Path],
                    labels: List[str],
                    out_path: Path,
                    comment: str,
    ...
    ax.set_xlabel(r"$\kappa_4$", loc='right')
    ...
    banner(ax, comment)
    scenI_idx = 1
    (p_ref, x_ref, y_ref) = curves[scenI_idx]
    lab_ref = labels[scenI_idx]
    ...
    if (L68 is not None) and (R68 is not None):
        ...
        if "omparison" in comment:
            ...
            txt = rf"$\kappa_4 \in [{L68:.1f},\,{R68:.0f}]$"
        ax.text(
            xmid, ytxt, txt,
        ...
        print(f"[WARN] Could not extract 68% interval for '{lab_ref}' (scan may not cross dNLL=0.5 on both sides).")
    fig.tight_layout()
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    ap.add_argument("--out", default="NLLscan_k4_compare_newscen.pdf", help="Output PDF path.")
...
def main(argv: List[str]) -> int:
    ...
    # Defaults
    default_files = [
        Path("/data/atlas/users/dingleyt/FCChh/trex/TRExFitter/hhh_SR_asimov_Hist_220925_k4_NoSyst_2bin/LHoodPlots/NLLscan_k4_curve.root"),
        Path("/data/atlas/users/dingleyt/FCChh/trex/TRExFitter/hhh_SR_asimov_Hist_220925_k4_ScenI_2bin/LHoodPlots/NLLscan_k4_curve.root"),
        Path("/data/atlas/users/dingleyt/FCChh/trex/TRExFitter/hhh_SR_asimov_Hist_220925_k4_ScenII_2bin/LHoodPlots/NLLscan_k4_curve.root"),
    ]
