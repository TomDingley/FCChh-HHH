...
sel_eta::sel_eta(float arg_max_eta) : m_max_eta(arg_max_eta) {};
...
std::vector<edm4hep::MCParticleData> getPromptLeptons(
...
{
    std::vector<edm4hep::MCParticleData> fromH;
    for (const auto& lepton : leptons) {
        const int orig = get_lepton_origin(lepton, mcparticles, ind);
        if (std::abs(orig) != 15) continue;  // only care about e from tau

        // climb to the tau ancestor of this lepton
        for (unsigned j = lepton.parents_begin; j != lepton.parents_end; ++j) {
            const int idxTau = ind.at(j);
            if (idxTau < 0 || (size_t)idxTau >= mcparticles.size()) continue;
            const auto& tau = mcparticles[idxTau];
            if (std::abs(tau.PDG) != 15) continue;

            // ask for the tau's origin
            const int tauOrig = get_lepton_origin(tau, mcparticles, ind);
            if (tauOrig == 25 || tauOrig == -25) {
                fromH.push_back(lepton);
                break; // don't duplicate this lepton
            }
        }
    }
    return fromH;
...
// non-prompt leptons
// Non-higgs decay leptons: keep leptons that are NOT from H directly
// and NOT from a tau whose origin is H.
std::vector<edm4hep::MCParticleData> getNonPromptLeptons(
    const ROOT::VecOps::RVec<edm4hep::MCParticleData>& mcparticles,
    const ROOT::VecOps::RVec<edm4hep::MCParticleData>& leptons,
    const ROOT::VecOps::RVec<int>& ind)
{
  std::vector<edm4hep::MCParticleData> notfromH;
  notfromH.reserve(leptons.size());

  for (size_t i = 0; i < leptons.size(); ++i) {
    const auto& lepton = leptons[i];

    // Where does THIS lepton come from?
    const int originPDG = get_lepton_origin(lepton, mcparticles, ind);
    // std::cout << "Lepton origin " << originPDG << std::endl;

    bool keep = true;

    // If the lepton's immediate origin is a Higgs, exclude it.
    if (std::abs(originPDG) == 25) {
      keep = false;
    }
    // If the lepton comes from a tau, check the tau's origin.
    else if (std::abs(originPDG) == 15) {
      bool foundTau = false;
      for (unsigned j = lepton.parents_begin; j != lepton.parents_end; ++j) {
        const int pidx = ind.at(j);
        const auto& parent = mcparticles.at(pidx);
        if (std::abs(parent.PDG) == 15) {
          foundTau = true;
          const int tauOrigin = get_lepton_origin(parent, mcparticles, ind);
          // std::cout << "Tau origin " << tauOrigin << std::endl;
          if (std::abs(tauOrigin) == 25) {
            keep = false; // tau from Higgs => exclude
          }
          break; // found the tau parent we needed
        }
      }
      // If we somehow didn't find the tau parent in the list, default to keep=true.
      (void)foundTau;
    }
    // else: origin is not tau and not H directly -> keep

    if (keep) notfromH.push_back(lepton);
  }
