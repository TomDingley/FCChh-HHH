...
ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> get_leading_n(ROOT::VecOps::RVec<edm4hep::ReconstructedParticleData> in, int n) {
  // Sort input by descending pt and return up to 4 leading jets
  std::vector<std::pair<float, size_t>> pt_indices;
  for (size_t i = 0; i < in.size(); ++i) {
    float pt = std::sqrt(in[i].momentum.x * in[i].momentum.x + in[i].momentum.y * in[i].momentum.y);
  ...
  for (size_t i = 0; i < pt_indices.size() && i < n; ++i) {
    result.push_back(in[pt_indices[i].second]);
  }
  return result;
}
