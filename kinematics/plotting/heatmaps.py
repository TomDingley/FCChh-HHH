...
def plot_hist2d_heatmaps(
    ...
    *,
    ...
    for vx, vy in HEATMAP_PAIRS:
        ...
        if norm.lower() == "log":
            # Guard against non-positive bins for LogNorm
            Hplot = np.where(H > 0, H, np.nan)
            mappable = ax.pcolormesh(
                xedges, yedges, Hplot.T,
                cmap=cmap, shading="auto",
                norm=LogNorm(vmin=vmin if vmin else max(np.nanmin(Hplot), 1e-12),
        ...
        cbar = fig.colorbar(mappable, ax=ax, label=cbar_label)
